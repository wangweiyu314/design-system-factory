...
引擎只带通用骨架，布局只有 `mixed` 一套。
**目标系统大概率还有自己特有的布局 / 页面类型**——多数中后台至少还需要一套侧边导航布局。

不要靠猜缺什么，让生成器报：

```bash
$NODE <包>/scripts/new-page.js --type=list --name=测试 --layout=side
```

缺模板时它不会干报错，而是打印完整创建引导：

```text
▸ layout/side-layout.html
  派生自　：scripts/layout/mixed-layout.html（同一套骨架，按需去掉通栏顶栏）
  结构锚点：保留 <main class="gf-content">；有面包屑时它必须是 .gf-content 的首子元素
  类名前缀：沿用 <prefix>-，不要另起一套
  落位　　：<包>/scripts/layout/side-layout.html

自检：node <包>/scripts/check-tokens.js <包> --strict
```
...
| 项 | 原因 |
| :-- | :-- |
| 派生自已有模板 | 自带正确的 `:root` 结构与 Token 引用，比从零写少踩绝大多数坑 |
| 结构锚点 | `[E4] 框架结构校验` 会查（面包屑须为内容区首子元素、`.gf-content` padding-top 为 0 等），不满足直接拦截 |
| 类名前缀一致 | 前缀是全库替换的锚点，另起一套会导致后续批量替换漏掉这个文件 |
| 落位路径 | `new-page.js` 按固定路径找模板，放错位置等于没沉淀 |

**沉淀后**：把新模板登记进 `SKILL.md` 的模板清单与 `scripts/README.md` 表格，
否则下一个人不知道它存在。

**反过来同样重要**：确实不需要的模板，就从文档清单里删掉，
别留「登记了但文件不存在」的条目——那正是这次要修的问题：
文档承诺的能力代码做不到，比没有文档更糟。
